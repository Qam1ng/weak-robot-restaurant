...
func _on_backend_dialogue_completed(_result: int, code: int, _headers: PackedStringArray, body: PackedByteArray, http: HTTPRequest, request_id: String, fallback: String) -> void:
	...
	if utterance == "":
		_emit_dialogue_fallback(request_id, str(meta.get("status", "empty_content")), fallback)
		return
	directed_utterance_generated.emit(request_id, utterance, meta)

func _emit_dialogue_fallback(request_id: String, status: String, fallback: String, http_code: int = -1) -> void:
	var meta := {
		"provider": "fallback",
		"status": status,
		"fallback": fallback
